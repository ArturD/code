...
#define ASSERTEQLLD( A, B ) { if( (A)!=(B) ) { fprintf(stderr, " ASSERT FAILED (%s==%s) (%lld==%lld) %s %d %s\n", #A,#B,(A),(B), __FILE__, __LINE__, __FUNCTION__); exit(1); } }
#define ASSERTEQ( A, B ) { if( (A)!=(B) ) { fprintf(stderr, " ASSERT FAILED (%s==%s) (%d==%d) %s %d %s\n", #A,#B,(A),(B), __FILE__, __LINE__, __FUNCTION__); exit(1); } }
...
void test_tree() {
  Tree< long long, less<long long> > T(8,-1);
  ASSERT( T.get(0,7)==-1 );
  ASSERT( T.get(1,1)==-1 );
  ...
  T.set(0,10);
  ASSERTEQ( T.getind(0,7), 0 );
  ASSERTEQLLD( T.get(0), 10ll );
  ASSERTEQLLD( T.get(0,7), 10ll );
  ASSERT( T.get(1,1)== 1 );
  ASSERT( T.get(0,0)== 10 );
  ASSERT( T.get(1,2)== 2 );
  ASSERT( T.get(2,2)== 2 );
  ...
  Tree< long long, less<long long> > T2(10,-inf);
  //100 -5 1 -10 1 -10 1 -5 173
  T2.set(1,100);
  T2.set(2,-5);
  T2.set(3,1);
  T2.set(4,-10);
